...
/// [CustomSearchDelegate.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [CustomSearchDelegate] can only be associated with one active [customShowSearch]
/// call. Call [CustomSearchDelegate.close] before re-using the same delegate instance
/// for another [customShowSearch] call.
///
...
/// The animation provided by [CustomSearchDelegate.transitionAnimation] can be used
...
/// See also:
///
///  * [CustomSearchDelegate] to define the content of the search page.
...
/// Delegate for [customShowSearch] to define the content of the search page.
///
/// The search page always shows an [AppBar] at the top where users can
/// enter their search queries. The buttons shown before and after the search
/// query text field can be customized via [CustomSearchDelegate.buildLeading]
/// and [CustomSearchDelegate.buildActions]. Additionally, a widget can be placed
/// across the bottom of the [AppBar] via [CustomSearchDelegate.buildBottom].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [CustomSearchDelegate.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [CustomSearchDelegate.buildResults].
///
/// [CustomSearchDelegate.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [CustomSearchDelegate.showResults]
/// and you can go back to showing the suggestions by calling
/// [CustomSearchDelegate.showSuggestions].
///
/// Once the user has selected a search result, [CustomSearchDelegate.close] should be
/// called to remove the search page from the top of the navigation stack and
/// to notify the caller of [customShowSearch] about the selected search result.
///
/// A given [CustomSearchDelegate] can only be associated with one active [customShowSearch]
/// call. Call [CustomSearchDelegate.close] before re-using the same delegate instance
/// for another [customShowSearch] call.
...
abstract class CustomSearchDelegate<T> {
  ...
  /// communicate the result back to the initial caller of [customShowSearch].
  ...
  /// to [customShowSearch] that launched the search initially.
...
class _SearchPageState<T> extends State<_SearchPage<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    final InputDecoration? inputDecoration = widget.delegate.searchFieldDecoration;
        ...
        child: Scaffold(
          appBar: AppBar(
            ...
            title: TextField(
              ...
              decoration: inputDecoration?? InputDecoration(hintText: searchFieldLabel),
            ),
